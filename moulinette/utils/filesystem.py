...
def read_file(file_path):
    ...
    """
    assert isinstance(file_path, str), "Error: file_path '%s' should be a string but is of type '%s' instead" % (file_path, type(file_path))
...
def write_to_file(file_path, data, file_mode="w"):
    ...
    """
    assert isinstance(data, str) or isinstance(data, list), "Error: data '%s' should be either a string or a list but is of type '%s'" % (data, type(data))
    ...
    if not isinstance(data, str):
        for element in data:
            assert isinstance(element, str), "Error: element '%s' should be a string but is of type '%s' instead" % (element, type(element))
        data = '\n'.join(data)
